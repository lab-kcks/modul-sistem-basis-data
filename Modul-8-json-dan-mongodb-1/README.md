...
Buka berkas MSI yang telah diunduh, klik "Next" pada tampilan awal instalasi. Selanjutnya, setujui `End-User License Agreement` dan klik "Next".

Setelah itu, pilih opsi setup **Complete**.

<img src="./img/2.png" />

Kemudian, pilih `Run service as Network Service user` dan salin path dari *data directory*. Klik "Next" dan lanjutkan proses instalasi.

<img src='./img/1.png' />

Langkah berikutnya adalah melakukan konfigurasi *environment variable*. Temukan lokasi instalasi MongoDB pada perangkat Anda dan salin path direktori `/bin`.

<img src='./img/3.png' />

Tambahkan path tersebut ke *environment variable* pada sistem Anda melalui:

`Properties >> Environment Variable >> System Variable >> Path >> Edit Environment Variable`

<img src='./img/4.png' />

### Menjalankan MongoDB Server dan Mengakses MongoDB Shell

Setelah berhasil mengkonfigurasi *environment variable*, MongoDB server dapat dijalankan dengan membuka terminal dan mengeksekusi perintah:

```shell
mongod
````

Umumnya, pada saat pertama kali menjalankan, pengguna mungkin menghadapi galat seperti `C:/data/db not found`.

Jika galat tersebut muncul, buatlah folder yang dimaksud secara manual pada direktori yang ditunjukkan. Setelah itu, coba jalankan kembali perintah `mongod`.

Tampilan berikut mengindikasikan bahwa MongoDB server telah berhasil berjalan:

\<img src='./img/5.png' /\>

Selanjutnya, MongoDB Shell dapat diakses untuk melakukan kueri terhadap basis data dengan membuka terminal baru dan menjalankan perintah:
...
Dalam MongoDB, *collection* dapat dianalogikan sebagai grup dokumen, atau serupa dengan skema dalam basis data relasional, meskipun *collection* pada MongoDB pada dasarnya tidak memberlakukan aturan skema yang kaku.

*Collection* dapat dibuat secara eksplisit dengan menjalankan:
...
Penjelasan:
  ...
  - Metode `createCollection()` digunakan untuk membuat *collection* secara eksplisit.
  - Output yang umum adalah `{ "ok" : 1 }`, yang menandakan operasi berhasil.

> Perintah `createCollection()` bersifat opsional, karena *collection* dapat terbentuk secara otomatis ketika dokumen pertama disisipkan.

\<img src='./img/7.jpeg' /\>

#### Operasi Insert

Berikut adalah berbagai metode untuk memasukkan data baru ke dalam *collection*. Setelah basis data dan *collection* siap (atau akan dibuat secara implisit), proses penyisipan dokumen dapat dimulai. Dokumen dalam MongoDB memiliki struktur serupa objek JSON.

1.  **Metode `insertOne()` di MongoDB**

    Digunakan untuk menyisipkan satu dokumen ke dalam *collection*.
...
2.  **Metode `insertMany()` di MongoDB**
    ...
    Digunakan untuk menyisipkan banyak dokumen sekaligus ke dalam *collection*. Metode ini lebih efisien dibandingkan `insertOne()` yang dijalankan berulang kali untuk volume data yang besar.
...
Pertama, dapat diketahui terlebih dahulu daftar *collection* yang ada dalam basis data aktif dengan menjalankan:
...
1.  **Kueri `findOne()`**
    ...
    Digunakan untuk mengambil satu dokumen dari *collection* tertentu berdasarkan kriteria yang spesifik.
    ...
    (*id: 0 berarti field \_id tidak ditampilkan, field lain dengan nilai 1 akan ditampilkan*)
