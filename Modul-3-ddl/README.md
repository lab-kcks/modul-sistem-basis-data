...
| 2. | TIME | Menyimpan data waktu dengan format (jam:menit:detik). | -838:59:59 s/d +838:59:59 |
| 3. | DATETIME | Menyimpan data tanggal dan waktu. | 1000-01-01 00:00:00 s/d 9999-12-31 23:59:59 |
...
> 💡 **Tips:** Gunakan `IF NOT EXISTS` saat membuat database atau tabel untuk mencegah error jika objek tersebut sudah ada sebelumnya.
>
> ```sql
...
> ```
>
> Tanpa `IF NOT EXISTS`, MySQL akan menampilkan error jika database atau tabel dengan nama yang sama sudah ada.
...
CREATE TABLE tryout (
    ...
    Jenis_Tryout VARCHAR(50)
);

CREATE TABLE result_tryout (
...
Constraint yang digunakan untuk mengidentifikasi setiap baris secara unik dalam suatu tabel. Jenis constraint ini juga secara tidak langsung menggunakan kombinasi `NOT NULL` dan `UNIQUE` dikarenakan kolom yang menggunakan constraint ini tidak boleh NULL dan harus berbeda antar data. Setiap tabel hanya dapat memiliki satu `PRIMARY KEY`, namun `PRIMARY KEY` dapat terdiri dari satu atau lebih kolom (composite primary key).
